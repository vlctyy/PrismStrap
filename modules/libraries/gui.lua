...
if not isfile('Bloxstrap/modules/configuration/'.. presets.config.. '.json') then
    writefile('Bloxstrap/modules/configuration/'.. presets.config.. '.json', '{}')
end
...
local Funcs = {} do
	...
	function Funcs:FireCallback(tab, api, ...)
...
function redzlib:MakeWindow(Configs)
	...
	function Window:MakeTab(paste, Configs)
		...
		local TemporailyObjects = {}
		function Tab:RemoveObject(Name)
		    if TemporailyObjects[Name] then
		        TemporailyObjects[Name]:Destroy()
		        TemporailyObjects[Name] = nil
		    end
		end
		...
		function Tab:AddButton(Configs)
			...
			local BDescription = Configs.Desc or Configs.Description or Configs.Tooltip or ""
		...
		function Tab:AddToggle(Configs)
			...
			local TDesc = Configs.Desc or Configs.Description or Configs.Tooltip or ""
			local Callback = Funcs:GetCallback(Configs, 3)
			local Flag = Configs[4] or Configs.Flag or false
			
			local value = configsettings[TName]
			
			local api = {
			    Name = TName,
			    Default = value ~= nil and value
			}
			if api.Default == nil and Configs.Default ~= nil then
			    api.Default = Configs.Default
			 end
	    
			if CheckFlag(Flag) then api.Default = GetFlag(Flag) end
			...
			local function SetToggle(Val)
				...
				api.Default = Val
				WaitClick = Val
				SetFlag(Flag, api.Default)
				Funcs:FireCallback(Callback, api, api.Default)
				
				
				if tostring(api.Default) == "true" then
			...
			if api.Default ~= nil then
		      task.spawn(SetToggle, api.Default)
		  end
			
			Button.Activated:Connect(function()
				SetToggle(not api.Default)
			end)
		...
		function Tab:AddDropdown(Configs)
			...
			local DDesc = Configs.Desc or Configs.Tooltip or Configs.Description or ""
			...
			local api = {
			    Name = DName,
			    Default = configsettings[DName]
			}
			
			if api.Default == nil and OpDefault then
			    api.Default = OpDefault
			    --print('dropdown new cfg')
			 end
			...
			local AddNewOptions, GetOptions, AddOption, RemoveOption, Selected do
				local Default = type(api.Default) ~= "table" and {api.Default} or api.Default
				...
				local function CallbackSelected()
					...
					api.Default = Selected
					Funcs:FireCallback(Callback, api, Selected)
				end
				...
				if api.Default ~= nil then CallbackSelected() end
				UpdateSelected()
			end
		...
		function Tab:AddSlider(Configs)
			...
			local SDesc = Configs.Desc or Configs.Tooltip or Configs.Description or ""
			...
			local api = {
			    Name = SName,
			    Default = configsettings[SName]
			}
			if api.Default == nil then
			    api.Default = Configs.Default or Min
			end
		...
		function Tab:AddTextBox(Configs)
			...
			Tab:RemoveObject(TName)
			local TDesc = Configs.Desc or Configs.Tooltip or Configs.Description or ""
			...
			if api.Default == nil then
			    api.Default = TDefault
			end
			
			if type(api.Default) ~= "string" or api.Default:gsub(" ", ""):len() < 1 then
				api.Default = tostring(api.Default)
			end
			
			local Button, LabelFunc = ButtonFrame(Container, TName, TDesc, UDim2.new(1, -38))
			
			TemporailyObjects[TName] = Button
			...
			function TextBox:Set(Val1: string)
				Text = Val1
				api.Default = Val1
				Funcs:FireCallback(Callback, api, Val1)
				TextBoxInput.Text = Val1
			end
			if api.Default ~= nil then
			    TextBox:Set(api.Default)
			end
